...
//
// The version of atlbase.h which shipped with Visual Studio .NET 2003 
// uses some deprecated functions.  Warning C4995 is disabled 
...
BOOL CAudioCapDlg::OnInitDialog()
{
    ...
    {
        ReleaseCapture();
        PostQuitMessage(0); // Can't capture audio, so exit the app
    }
...
HRESULT CAudioCapDlg::HandlePlaybackGraphEvent(void)
{
    ...
    // Read all events currently on the event queue
    while(SUCCEEDED(m_pMEPlayback->GetEvent(&evCode, &evParam1, &evParam2, 0)))
    {
        // Free event parameters to prevent memory leaks
...
HRESULT CAudioCapDlg::RenderPreviewStream()
{
    ...
    {
        (void)StringCchCopyW(wszFilter, NUMELMS(wszFilter), T2W(TEXT("Smart Tee\0")));
    ...
    (void)StringCchCopyW(wszFilter, NUMELMS(wszFilter), T2W(TEXT("Audio Renderer\0")));
