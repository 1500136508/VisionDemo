...
HRESULT CBasePropSetter::SetNumericVal(const TCHAR *szName, DWORD dwVal, int iBase = 10)
{
    ...
    if (iBase == 16)
    {
        hr = StringCchPrintf(szVal, NUMELMS(szVal), TEXT("%#06X\0"), dwVal);
    }
    else if (iBase == 10)
    {
        hr = StringCchPrintf(szVal, NUMELMS(szVal), TEXT("%d\0"), dwVal);
    }
    else
    {
        return E_INVALIDARG;
...
HRESULT CPropSetter::OnInitDialog()
{
    ...
    {
        LRESULT lr = SendMessage(hList, LB_ADDSTRING, 0, (LPARAM)OLE2T(*iter));
    }
