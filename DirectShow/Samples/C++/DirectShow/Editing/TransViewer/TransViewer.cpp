...
void    GraphEventCallback(long evCode, LONG_PTR Param1, LONG_PTR Param2);
...
// Utility function for getting the Media directory
...
HRESULT SetFileName(int nSource)
{
    ...
    TCHAR tszInitDir[MAX_PATH] = TEXT("\0");

    if (SUCCEEDED(DXUtil_GetDXSDKMediaPathCch( tszInitDir, MAX_PATH)))
    {
        const TCHAR* strVideo = _T("Video");
        if( lstrlen(tszInitDir) + lstrlen(strVideo) < MAX_PATH )
            hr = StringCchCat( tszInitDir, MAX_PATH, strVideo );
    }
    ...
    ofn.lpstrInitialDir = tszInitDir;
...
HRESULT PickSolidColor(int nSource)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        HWND hLabel = GetDlgItem(g_hwnd, (nSource == 1 ? IDC_CLIP1_CAPTION : IDC_CLIP2_CAPTION));
        hr = StringCchPrintf(szColorName, NUMELMS(szColorName), TEXT("0x%06X\0"), cc.rgbResult);
