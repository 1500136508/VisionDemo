...
public:
    ...
    BOOL CopyBitmap( double dblSampleTime, BYTE * pBuffer, long lBufferSize )
    {
        ...
        HRESULT hr = StringCchPrintf( m_szSnappedName, NUMELMS(m_szSnappedName), TEXT("%sStillCap%4.4ld.bmp\0"), 
                  m_szCapDir, pOwner->m_nCapTimes );

        hr = StringCchPrintf( m_ShortName, NUMELMS(m_ShortName), TEXT("StillCap%4.4ld.bmp\0"), 
                  pOwner->m_nCapTimes );
    ...
    void PlaySnapSound(void)
    {
        ...
        {
            HRESULT hr = StringCchCat( szSound, NUMELMS(szSound), szFileToPlay );
...
void CStillCapDlg::OnSnap() 
{
    ...
    HRESULT hr = StringCchPrintf( szFilename, NUMELMS(szFilename),TEXT("%sStillCap%04d.avi\0"), (LPCTSTR) CapDir, m_nCapTimes );
    hr = StringCchPrintf( szFile, NUMELMS(szFile),TEXT("StillCap%04d.avi\0"), m_nCapTimes );
