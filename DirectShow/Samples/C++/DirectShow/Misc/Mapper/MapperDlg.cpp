...
BOOL CMapperDlg::OnInitDialog()
{
    ...
    // Go ahead and perform a default search with the default parameters
    OnButtonSearch();
...
BOOL CMapperDlg::InitializeTooltips()
{
    ...
    if (!m_pToolTip)
    {
       TRACE(TEXT("Failed to allocate ToolTip control\n"));
       return FALSE;
    }
...
BOOL CMapperDlg::PreTranslateMessage(MSG* pMsg) 
{
    ...
    // Pass the message along to the dialog 
...
int CMapperDlg::GetFilenameByCLSID(REFCLSID clsid, TCHAR *szFile, size_t len)
{
    HRESULT hr;
    LPOLESTR strCLSID;
    int rc=0;
    ...
    if(SUCCEEDED(hr))
    {
        ...
        if (rc == ERROR_SUCCESS)
        {
            ...
            if (rc == ERROR_SUCCESS)
                hr = StringCchPrintf(szFile, len, TEXT("%s\0"), szFilename);
...
void CMapperDlg::AddMerit(TCHAR *szInfo, size_t len, DWORD dwMerit)
{
    TCHAR szMerit[32];
    HRESULT hr;
    ...
    {
        case MERIT_NORMAL:
             hr = StringCchCopy(szMerit, NUMELMS(szMerit), TEXT("MERIT_NORMAL\0"));
             break;     // Assume normal
        case MERIT_PREFERRED:
             hr = StringCchCopy(szMerit, NUMELMS(szMerit), TEXT("MERIT_PREFERRED\0"));
             break;
        case MERIT_UNLIKELY:
             hr = StringCchCopy(szMerit, NUMELMS(szMerit), TEXT("MERIT_UNLIKELY\0"));
             break;
        case MERIT_DO_NOT_USE:
             hr = StringCchCopy(szMerit, NUMELMS(szMerit), TEXT("MERIT_DO_NOT_USE\0"));
             break;
        case MERIT_SW_COMPRESSOR:
             hr = StringCchCopy(szMerit, NUMELMS(szMerit), TEXT("MERIT_SW_COMPRESSOR\0"));
             break;
        case MERIT_HW_COMPRESSOR:
             hr = StringCchCopy(szMerit, NUMELMS(szMerit), TEXT("MERIT_HW_COMPRESSOR\0"));
             break;
        
        default:
             hr = StringCchPrintf(szMerit, NUMELMS(szMerit), TEXT("0x%08x\0"), dwMerit);
             break;
    }
...
void CMapperDlg::AddFilter(const TCHAR *szFilterName, const GUID *pCatGuid, DWORD dwMerit)
{
    ...
    hr = StringCchPrintf(szInfo, NUMELMS(szInfo), TEXT("%s\t\0"), szFilterName);

    // Add the filter's merit value
    AddMerit(szInfo, NUMELMS(szInfo), dwMerit);

    // Get the filter's server name from the registry
    int rc = GetFilenameByCLSID(*pCatGuid, szFilename, NUMELMS(szFilename));

    // Add the filter's server name
    if (rc == ERROR_SUCCESS)
    {
        hr = StringCchCat(szInfo, NUMELMS(szInfo), TEXT("\t\0"));
        hr = StringCchCat(szInfo, NUMELMS(szInfo), szFilename);
    }
...
void CMapperDlg::OnButtonSearch() 
{
    ...
    hr = StringCchPrintf(szNumFilters, NUMELMS(szNumFilters), TEXT("Filters Matching Query : %d\0"), nFilters);
