...
void CDMOEnumDlg::SetNumClasses(int nClasses)
{
    ...
    (void)StringCchPrintf(szClasses, NUMELMS(szClasses), TEXT("%s (%d found)\0"), STR_CLASSES, nClasses);
...
void CDMOEnumDlg::SetNumFilters(int nFilters)
{
    ...
    if (nFilters)
        (void)StringCchPrintf(szFilters, NUMELMS(szFilters), TEXT("%s (%d found)\0"), STR_FILTERS, nFilters);
    else
    {
        _tcsncpy(szFilters, STR_FILTERS, 63);
        szFilters[63] = 0;
    }
    ...
    const GUID *pCatGuid)
{
    ...
    int nLocation  = m_FilterList.AddString(szFilterName);
    if ((nLocation == LB_ERR) || (nLocation == LB_ERRSPACE))
        return;

    int nIndexNew = m_FilterList.FindStringExact(-1, szFilterName);
    if ((nIndexNew == LB_ERR) || (nIndexNew == LB_ERRSPACE))
        return;
    ...
    if (pclsid)
    {
        *pclsid = *pCatGuid;
        ...
        // Add the CLSID to the list item's data.  On failure, set to zero.
        if (LB_ERR == m_FilterList.SetItemDataPtr(nIndexNew, pclsid))
            m_FilterList.SetItemDataPtr(nIndexNew, 0);
    }
}
...
void CDMOEnumDlg::GetTypeSubtypeString(TCHAR *szCLSID, size_t len, DMO_PARTIAL_MEDIATYPE& aList)
{
    HRESULT hr;
    ...
    hr = StringCchPrintf(szCLSID, len, TEXT("%s\t%s\0"), (LPCTSTR) strType, (LPCTSTR) strSubtype);
}
    ...
    if(SUCCEEDED(hr))
    {
        ...
        if (rc == ERROR_SUCCESS)
        {
            ...
            if (rc == ERROR_SUCCESS)
            {
                ...
                hr = StringCchPrintf(szFilename, NUMELMS(szFilename), TEXT("%s\0"), pbFilename);
...
void CDMOEnumDlg::GetFormatString(TCHAR *szFormat, DMO_MEDIA_TYPE *pType, int nFormatSize)
{
    int i=0;
    ...
    while (rgng[i].pguid != 0)
    {
        ...
        {
            _tcsncpy(szFormat, rgng[i].psz, nFormatSize-1);
            szFormat[nFormatSize-1] = TEXT('\0');
            return;
        }
        i++;
    }
    ...
    (void)StringCchPrintf(szFormat, nFormatSize, TEXT("Format_None\0"));
}

void CDMOEnumDlg::GetGUIDString(TCHAR *szString, GUID *pGUID, int nStringSize)
{
    int i=0;
    ...
    while (rgng[i].pguid != 0)
    {
        ...
        {
            _tcsncpy(szString, rgng[i].psz, nStringSize-1);
            szString[nStringSize-1] = TEXT('\0');
            return;
        }
        i++;
    }
    ...
    // If we got here, there was no match
    (void)StringCchCopy(szString, nStringSize, TEXT("GUID_NULL\0"));
}
...
void CDMOEnumDlg::OnSelchangeListInputStreams() 
{
    ...
    if (SUCCEEDED(hr))
    {
        GetFormatString(sz, &dmt, NUMELMS(sz));
        m_StrInFormat.SetWindowText(sz);

        GetGUIDString(sz, &dmt.majortype, NUMELMS(sz));
        m_StrInType.SetWindowText(sz);

        GetGUIDString(sz, &dmt.subtype, NUMELMS(sz));
