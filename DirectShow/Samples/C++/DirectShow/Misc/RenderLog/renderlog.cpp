...
HRESULT RenderClip(LPTSTR szFile)
{
    ...
    (void)StringCchCopyW(wFile, NUMELMS(wFile), T2W(szFile));
    wFile[MAX_PATH-1] = 0;
...
void SetRenderLog(void)
{
    ...
    (void)StringCchCopy(szLogFile, NUMELMS(szLogFile),DEFAULT_LOG_FILE);
    ...
    if (bSuccess)
    {
        (void)StringCchCopy(g_szRenderLog, NUMELMS(g_szRenderLog), ofn.lpstrFile);
...
void ViewRenderLog(void)
{
    ...
    if (GetFileAttributes(g_szRenderLog) != (DWORD) -1)
    {
        ...
        // that exists in the Windows system directory.  Allow for spaces in the path.

        // Prepend a quote to allow for paths with spaces
        szAppName[0] = TEXT('\"');   szAppName[1] = TEXT('\0');

        // Read system directory and form the full application path
        if (0 != GetSystemDirectory(szAppName+1, MAX_PATH - 20))
        {
            // Add the trailing quote to handle paths with spaces
            // (for example, "c:\Microsoft Windows\system32")
            hr = StringCchCat(szAppName, NUMELMS(szAppName), TEXT("\\notepad.exe\" \0"));
        }
        else
        {
            // If GetSystemDirectory fails, just use the executable name
            hr = StringCchCat(szAppName, NUMELMS(szAppName), TEXT("notepad.exe \0"));
        }
...
int PASCAL WinMain(HINSTANCE hInstC, HINSTANCE hInstP, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    if(ghApp)
    {
        ...
        (void)StringCchCopy(g_szRenderLog, NUMELMS(g_szRenderLog), DEFAULT_LOG_FILE);
