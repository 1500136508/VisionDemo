...
bool CDvdCore::BuildGraph()
{
    ...
    {
#ifdef UNICODE
        lstrcpyn(szwDiscPath, m_szDiscPath, MAX_PATH) ;
#else
                              ...
                              DWORD dwMaxText)
{
    ...
    if (IsBadWritePtr(pszStatusText, sizeof(*pszStatusText) * dwMaxText))
    {
...
bool CDvdCore::InitMessageSink()
{
    ...
    WNDCLASSEX wndclass = {
        ...
        0,
        m_hInstance,
        LoadIcon(m_hInstance, MAKEINTRESOURCE(IDI_APPICON)),
...
LRESULT CALLBACK CDvdCore::WndProc(HWND hWnd, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
    ...
    pCore = reinterpret_cast<CDvdCore *>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
...
#ifdef UNICODE
    lstrcpyn(szwFileName, szDirectory, MAX_PATH-1) ;
#else
...
bool CDvdCore::ChangeDvdRegion()
{
    ...
    #else
        strncpy(szDVDDriveA, szDVDDrive, 3);
    #endif  // UNICODE
    ...
    else   // Windows 98 platform
    {
        ...
        lstrcpyn(szAppName, szCmdLine, NUMELMS(szAppName));
...
bool CDvdCore::GetDriveLetter(TCHAR *pszDrive)
{
