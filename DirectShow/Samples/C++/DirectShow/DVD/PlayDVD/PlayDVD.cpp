...
void UpdateMainTitle(void)
{
    ...
    if (g_PlaybackRate == 1.0)
        ...
        hr = StringCchPrintfA(szPlaybackRate, NUMELMS(szPlaybackRate),"(Rate:%2.2f)", g_PlaybackRate);

    hr = StringCchCopy(szRate, NUMELMS(szRate), A2T(szPlaybackRate) );
    ...
    {
        hr = StringCchPrintf(szTimeChapter, NUMELMS(szTimeChapter), TEXT("[%d:%02d:%02d] [Title %d Chapter %d] \0"),
