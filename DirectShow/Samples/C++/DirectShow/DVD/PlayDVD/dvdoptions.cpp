...
HRESULT GetTitleInfo()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
            TCHAR szTitle[32];
            hr = StringCchPrintf(szTitle, NUMELMS(szTitle), TEXT("Title %d\0"), i);
...
HRESULT UpdateChapterCount(ULONG ulTitle)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
            TCHAR szChapter[32];
            hr = StringCchPrintf(szChapter, NUMELMS(szChapter), TEXT("Chapter %d\0"), i);
...
HRESULT GetAngleInfo()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        else
        {
            ...
            {
                TCHAR szAngle[32];
                hr = StringCchPrintf(szAngle, NUMELMS(szAngle), TEXT("Angle %d\0"), i);
...
HRESULT GetMenuLanguageInfo()
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        for (i=0; i < (int) ulLanguagesAvailable; i++)
        {
            ...
            hr = StringCchPrintf(szMenu, NUMELMS(szMenu), TEXT("%s (0x%x)\0"), szLang, g_pLanguageList[i]);
...
void ShowValidUOPS(TCHAR *szAttemptedAction)
{
    ...
    // Format the message
    hr = StringCchPrintf(szMsg, NUMELMS(szMsg), TEXT("Changing %s is not valid at this time (prohibited by DVD).\n\n")
             TEXT("The valid user operations (UOPS) allowed now are:\n\0"),
             szAttemptedAction);

#define HANDLE_UOP(c)                       \
        if ((g_ulValidUOPS & c) == 0)       \
            hr = StringCchCat(szMsg, NUMELMS(szMsg), _T("\t") _T(#c) _T("\n\0")); \
