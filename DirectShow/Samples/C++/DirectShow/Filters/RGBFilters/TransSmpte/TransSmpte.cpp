...
HRESULT CTransSmpte::Transform(IMediaSample *pSample)
{
    ...
    TCHAR szText[256];
    (void)StringCchPrintf( szText, NUMELMS(szText), TEXT("Frame %8.8d\0"), nFrames );
    ...
    if (m_dFPS != 0)
    {
        ...
        (void)StringCchPrintf( szText, NUMELMS(szText), TEXT("H:%2.2d M:%2.2d S:%2.2d F:%2.2d\0"), 
                  nHours, nMinutes, nSeconds, nFrames );
        bWorked = TextOut( m_DC, 10, 30, szText, _tcslen( szText ) );
    }
    else
    {
        (void)StringCchPrintf( szText, NUMELMS(szText), TEXT("H:%2.2d M:%2.2d S:%2.2d F:?? ??\0"), 
    ...
    if (m_dFPS != 0)
    {
        (void)StringCchPrintf( szText, NUMELMS(szText), TEXT("FPS = %d\0"), long( 1.0 / m_dFPS ) );
        bWorked = TextOut( m_DC, 10, 50, szText, _tcslen( szText ) );
    }
    else
    {
        (void)StringCchPrintf( szText, NUMELMS(szText), TEXT("FPS = ??\0") );
        bWorked = TextOut( m_DC, 10, 50, szText, _tcslen( szText ) );
    }
