...
void CStillViewDlg::InitMediaDirectory(void)
{
    ...
    if (theApp.m_lpCmdLine[0] == L'\0')
    {
        ...
        hr = StringCchCat( szDir, NUMELMS(szDir), TEXT("Misc") );
        ...
        if (lResult != 0)
        {
            if (GetWindowsDirectory(szDir, MAX_PATH))
            {
                if ((_tcslen(szDir) + STR_MEDIA_FOLDER_SIZE) < MAX_PATH)
                    hr = StringCchCat( szDir, NUMELMS(szDir), STR_MEDIA_FOLDER );
            }
            else
            {
    ...
    {
        _tcsncpy(szDir, theApp.m_lpCmdLine, NUMELMS(szDir));
        szDir[MAX_PATH-1] = 0;      // Ensure null-termination
    }

    TCHAR szPathMsg[MAX_PATH + 64];
    hr = StringCchPrintf(szPathMsg, NUMELMS(szPathMsg), TEXT("Media directory: %s\0"), szDir);
    m_StrMediaPath.SetWindowText(szPathMsg);

    // Save current directory name
    hr = StringCchPrintf(m_szCurrentDir, NUMELMS(m_szCurrentDir), TEXT("%s\0"), szDir);
...
LONG CStillViewDlg::GetDXMediaPath(TCHAR *szPath, DWORD dwPathSize)
{
    HKEY  hKey=0;
...
void CStillViewDlg::OnButtonSetMediadir() 
{
    ...
    int nLength = (int) _tcslen(szEditPath);
...
BOOL CStillViewDlg::DisplayFileInfo(LPTSTR szFile)
{
    ...
    if (GetFileInformationByHandle(hFile, &fi))
    {
        ...
        hr = StringCchPrintf(szScrap, NUMELMS(szScrap), TEXT("Date: %02d/%02d/%d\0"), 
        ...
        hr = StringCchPrintf(szScrap, NUMELMS(szScrap), TEXT("Time: %02d:%02d %s\0"), 
