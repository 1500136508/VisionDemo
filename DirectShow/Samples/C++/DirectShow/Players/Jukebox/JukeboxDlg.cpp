...
void CJukeboxDlg::InitMediaDirectory(void)
{
    ...
    {
        // If the media path is found, it will be returned in szDir
        lResult = GetDXMediaPath(szDir, sizeof(szDir));
        ...
        if (lResult != 0)
        {
            ...
            {
                if ((_tcslen(szDir) + STR_MEDIA_FOLDER_SIZE) < MAX_PATH)
                    (void)StringCchCat(szDir, MAX_PATH, STR_MEDIA_FOLDER );
            }
            else
            {
    ...
    {
        (void)StringCchCopy(szDir, NUMELMS(szDir), theApp.m_lpCmdLine);
        szDir[MAX_PATH-1] = 0;
    }

    TCHAR szPathMsg[MAX_PATH + 64];
    (void)StringCchPrintf(szPathMsg, NUMELMS(szPathMsg), TEXT("Media directory: %s\0"), szDir);
    m_StrMediaPath.SetWindowText(szPathMsg);

    // Save current directory name
    (void)StringCchPrintf(m_szCurrentDir, NUMELMS(m_szCurrentDir), TEXT("%s\0"), szDir);
...
LONG CJukeboxDlg::GetDXMediaPath(TCHAR *szPath, DWORD dwPathSize)
{
    ...
    HRESULT hr = StringCbCat( szPath, dwPathSize, STR_MEDIA_FOLDER );
    if (hr == S_OK)
    {
        return 0;
    }
...
void CJukeboxDlg::OnButtonSetMediadir()
{
    ...
    HRESULT hr = StringCchPrintf(szPathMsg, NUMELMS(szPathMsg), TEXT("Media directory: %s\0"), szEditPath);
    m_StrMediaPath.SetWindowText(szPathMsg);
    ...
    if (szEditPath[nLength - 1] != TEXT('\\'))
        hr = StringCchPrintf(m_szCurrentDir, NUMELMS(m_szCurrentDir), TEXT("%s\\\0"), szEditPath);
...
HRESULT CJukeboxDlg::EnumFilters (void)
{
    ...
    while(pEnum->Next(1, &pFilter, &cFetched) == S_OK)
    {
        ...
        else
        {
            ...
            hr = StringCchCopy(szName, NUMELMS(szName), W2T(FilterInfo.achName));
...
BOOL CJukeboxDlg::DisplayImageInfo(void)
{
    ...
    {
        TCHAR szSize[64];
        hr = StringCchPrintf(szSize, NUMELMS(szSize), TEXT("Image size: %d x %d\0"), lWidth, lHeight);
...
BOOL CJukeboxDlg::DisplayFileInfo(LPTSTR szFile)
{
    ...
    if (GetFileInformationByHandle(hFile, &fi))
    {
        ...
        hr = StringCchPrintf(szScrap, NUMELMS(szScrap), TEXT("File date: %02d/%02d/%d\0"),
    ...
    hr = StringCchPrintf(szScrap, NUMELMS(szScrap), TEXT("Size: %d bytes\0"), dwSizeLow);
...
HRESULT CJukeboxDlg::DisplayFileDuration(void)
{
    ...
    // Update the display
    TCHAR szDuration[24];
    hr = StringCchPrintf(szDuration, NUMELMS(szDuration), _T("%02dm:%02d.%03ds\0"), nMinutes, nSeconds, nMS);
...
void CJukeboxDlg::DisplayECEvent(long lEventCode, LONG_PTR lParam1, LONG_PTR lParam2)
{
...
#define HANDLE_EC(c)                              \
    case c:                                       \
        (void)StringCchCopy(szMsg, NUMELMS(szMsg), TEXT(#c));            \
        szMsg[31] = TEXT('\0');                   \
        break;
...
void CJukeboxDlg::UpdatePosition(REFERENCE_TIME rtNow)
{
    ...
    hr = StringCchPrintf(szPosition, NUMELMS(szPosition), _T("Position: %02dm:%02ds\0"), nMinutes, nSeconds);
...
void CJukeboxDlg::OnButtonGraphedit()
{
    ...
    HRESULT hr = StringCchPrintf(szFile, NUMELMS(szFile), TEXT("%s%s\0"), m_szCurrentDir, szFilename);
    ...
    if (lResult != 0)
    {
        hr = StringCchCopy(szPath, NUMELMS(szPath), TEXT("graphedt\0"));
    }
