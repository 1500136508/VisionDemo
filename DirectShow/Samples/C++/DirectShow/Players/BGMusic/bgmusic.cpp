...
void ShowCurrentFile(HWND hWnd)
{
    ...
    (void)StringCchPrintf(szMsg, NUMELMS(szMsg), _T("Playing %s\0"), g_szCurrentFile);
...
HRESULT SwapSourceFilter(void)
{
    ...
    (void)StringCchCopy(g_szCurrentFile, NUMELMS(g_szCurrentFile), pstrFiles[nNextFileIndex]);
    g_iNextFile++;
...
HRESULT DXUtil_GetDXSDKMediaPathCch( TCHAR* strDest, int cchDest )
{
    ...
    HRESULT hr = StringCchCopy( strDest, cchDest, TEXT("") );
    ...
    if( lstrlen(strDest) + lstrlen(strMedia) < cchDest )
        hr = StringCchCat( strDest, cchDest, strMedia );
    else
        return E_INVALIDARG;
...
HRESULT DXUtil_FindMediaFileCch( TCHAR* strDestPath, int cchDest, LPCTSTR strFilename )
{
    ...
    HRESULT hr = StringCchPrintf( strSearchFor, MAX_PATH, TEXT("media\\%s"), strFilename ); strSearchFor[MAX_PATH-1] = 0;
    ...
    (void)StringCchCopy( strLeafName, MAX_PATH, strFilename ); strLeafName[MAX_PATH-1] = 0;
    ...
    hr = StringCchPrintf( strLeafName, MAX_PATH, TEXT("media\\%s"), strFilename ); strLeafName[MAX_PATH-1] = 0;
    bFound = DXUtil_FindMediaSearchParentDirs( strDestPath, cchDest, TEXT("."), strLeafName );
    if( bFound )
        return S_OK;
    ...
    (void)StringCchCopy( strDestPath, cchDest, strFilename );
                                        ...
                                        TCHAR* strExePath, TCHAR* strExeName, TCHAR* strMediaDir )
{
    ...
    /* Search in ..\ */
    HRESULT hr = StringCchPrintf( strSearchPath, cchSearch, TEXT("..\\%s"), strLeaf ); strSearchPath[cchSearch-1] = 0;
    if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
        return true;

    /* Search in ..\..\ */
    hr = StringCchPrintf( strSearchPath, cchSearch, TEXT("..\\..\\%s"), strLeaf ); strSearchPath[cchSearch-1] = 0;
    if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
        return true;
    ...
    hr = StringCchPrintf( strSearchPath, cchSearch, TEXT("%s\\%s"), strExePath, strLeaf ); strSearchPath[cchSearch-1] = 0;
    if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
        return true;
    ...
    hr = StringCchPrintf( strSearchPath, cchSearch, TEXT("%s\\..\\%s\\%s"), strExePath, strExeName, strLeaf ); strSearchPath[cchSearch-1] = 0;
    if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
        return true;
    ...
    hr = StringCchPrintf( strSearchPath, cchSearch, TEXT("%s%s"), strMediaDir, strLeaf ); strSearchPath[cchSearch-1] = 0;
    if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
        return true;
...
bool DXUtil_FindMediaSearchParentDirs( TCHAR* strSearchPath, int cchSearch, TCHAR* strStartAt, TCHAR* strLeafName )
{
    ...
    {
        HRESULT hr = StringCchPrintf( strFullFileName, MAX_PATH, TEXT("%s\\%s"), strFullPath, strLeafName ); strFullFileName[MAX_PATH-1] = 0;
        ...
        hr = StringCchPrintf( strSearch, MAX_PATH, TEXT("%s\\.."), strFullPath ); strSearch[MAX_PATH-1] = 0;
