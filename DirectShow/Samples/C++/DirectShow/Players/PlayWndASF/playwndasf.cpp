...
HRESULT HandleNoRights( HRESULT hrStatus, WCHAR *wszChallengeURL)
{
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        hr = StringCchPrintfW(szURL, NUMELMS(szURL), L"%s&filename=%s&embedded=false", bstrChallengeURL, pszEscapedURL);
...
void UpdateMainTitle(void)
{
    ...
    else
    {
        ...
        hr = StringCchPrintf(szTitle, NUMELMS(szTitle), TEXT("%s [%s] %s%s"),
                szFile,
...
HRESULT HandleGraphEvent(void)
{
    ...
    while(SUCCEEDED(pME->GetEvent(&evCode, &evParam1, &evParam2, 0)))
    {
        ...
        {
            Msg(TEXT("Playback error (EC_ERRORABORT - 0x%08lx). Aborting...\r\n"), static_cast<LONG>(evParam1));

            if (FAILED(hr = pMC->Stop()))
                Msg(TEXT("Failed(0x%08lx) to stop media clip!\r\n"), hr);

            hr = static_cast<HRESULT>(evParam1);
            break;
        }
        else if( EC_WMT_EVENT == evCode )
        {
            AM_WMT_EVENT_DATA *pEventData = reinterpret_cast<AM_WMT_EVENT_DATA *>(evParam2);
            hr = HandleWMTEvent(pEventData, static_cast<LONG>(evParam1));
        }
    }
