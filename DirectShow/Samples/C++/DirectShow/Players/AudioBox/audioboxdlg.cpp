...
void CAudioboxDlg::InitMediaDirectory(void)
{
    ...
    if (theApp.m_lpCmdLine[0] == L'\0')
    {
        ...
        if (FALSE == InitCustomDirectory(CSIDL_MYMUSIC))
        {
            ...
            lResult = GetDXMediaPath(szDir, NUMELMS(szDir));
            ...
            if (lResult != 0)
            {
                ...
                {
                    HRESULT hr = StringCchCat(szDir, NUMELMS(szDir), _T("\\Media\\\0") );
                    if (hr != S_OK)
                    {
                        return;
                    }
        ...
        {
            (void)StringCchCopy(szDir, MAX_PATH, m_szCurrentDir);
    ...
    else
    {
        ...
        (void)StringCchCopy(szDir, MAX_PATH, theApp.m_lpCmdLine);
        szDir[MAX_PATH-1] = 0;      // Ensure null-termination
    }

    // Save current directory name for later use
    (void)StringCchCopy(m_szCurrentDir, NUMELMS(m_szCurrentDir), szDir);
...
LONG CAudioboxDlg::GetDXMediaPath(TCHAR *szPath, size_t len)
{
    HKEY  hKey;
    ...
    (void)StringCchCat( szPath, len, _T("\\Media\\\0") );
    return 0;
}
...
void CAudioboxDlg::FillDirList(LPTSTR pszRootDir)
{
    ...
    (void)StringCchCopy(szDir, MAX_PATH, pszRootDir);
    szDir[MAX_PATH-1] = 0;  // Ensure NULL termination
                   ...
                   DDL_DIRECTORY | DDL_EXCLUSIVE))
    {
        ...
        (void)StringCchCopy(m_szActiveDir, NUMELMS(m_szActiveDir), pszRootDir);
...
void CAudioboxDlg::OnSelectFile()
{
    ...
    else
    {
        ...
        hr = StringCchPrintf(szTitle, NUMELMS(szTitle),TEXT("AudioBox - %s\0"), szFilename);
        ...
        case WM_COMMAND:
            switch (wParam)
            {
                ...
                case IDM_SDK_MEDIA:
                    {
                        TCHAR szDir[MAX_PATH];
                        if (!GetDXMediaPath(szDir, MAX_PATH))
                            FillFileList(szDir);
                        else
                ...
                case IDM_WINDOWS_MEDIA:
                    {
                        ...
                        if (GetWindowsDirectory(szDir, MAX_PATH))
                        {
                            (void)StringCchCat(szDir, MAX_PATH, _T("\\Media\\\0") );
                            FillFileList(szDir);
                        }
                        else
...
void CAudioboxDlg::OnButtonSetMediadir()
{
    ...
    if (szEditPath[nLength - 1] != TEXT('\\'))
        (void)StringCchPrintf(m_szCurrentDir, NUMELMS(m_szCurrentDir),TEXT("%s\\\0"), szEditPath);
...
BOOL CAudioboxDlg::InitCustomDirectory(int nCustomDir)
{
    ...
    if (SHGetSpecialFolderPath(GetSafeHwnd(), szFolder, nCustomDir, 0))
    {
        ...
        (void)StringCchCopy(m_szCurrentDir, NUMELMS(m_szCurrentDir), szFolder);
        m_szCurrentDir[NUMELMS(m_szCurrentDir) - 1] = 0;  // Ensure NULL termination
        return TRUE;
    }
    else
    ...
    if (nIndex == emFavorites && !bSysMenu)
    {
        ...
        if(lResult == ERROR_SUCCESS)
        {
                                ...
                                &dwType, (BYTE *)&dwValueData, &dwValueDataSize))
            {
                ...
                (void)StringCchCopy(g_szFavorites[dwIndex], NUMELMS(g_szFavorites[dwIndex]), szValueName);
...
void CAudioboxDlg::GetShortName(TCHAR *pszFullPath, TCHAR *pszFolder)
{
    ...
    (void)StringCchCopy(szPath, MAX_PATH, pszFullPath);
    ...
    for (int i=nLength-1; i>=0; i--)
    {
        ...
        {
            szPath[i] = 0;
            (void)StringCchCopy(pszFolder, MAX_PATH, &szPath[i+1]);
            bSetFilename = TRUE;
            break;
        }
    }
    ...
    if (!bSetFilename)
        (void)StringCchCopy(pszFolder, MAX_PATH, pszFullPath);
}
