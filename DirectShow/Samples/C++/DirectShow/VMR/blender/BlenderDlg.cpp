...
HRESULT CBlenderDlg::InitializeVideo(void)
{
    ...
    (void)StringCchCopyW(wFile1, NUMELMS(wFile1), T2W(m_szFile1));
    (void)StringCchCopyW(wFile2, NUMELMS(wFile2), T2W(m_szFile2));
...
void CBlenderDlg::DisplayAlpha(int nStreamID)
{
    ...
    (void)StringCchPrintfA(szAnsiLabel, NUMELMS(szAnsiLabel),"(%02.2f)\0", strParam[nStreamID].Alpha);
    (void)StringCchCopy(szLabel, NUMELMS(szLabel), A2T(szAnsiLabel) );
...
void CBlenderDlg::DisplayCoordinates(int nStreamID, NORMALIZEDRECT& r)
{
    ...
    // The wsprintf method doesn't support floating point, so use the
    // ANSI version and convert to UNICODE if necessary.
    HRESULT hr = StringCchPrintfA(szAnsiLabel, NUMELMS(szAnsiLabel), "Stream %d  (%02.2f x %02.2f) at (%02.2f, %02.2f)\0",
             nStreamID, (float) (r.right - r.left), (float)(r.bottom - r.top),
             r.left, r.top);
    hr = StringCchCopy(szLabel, NUMELMS(szLabel), A2T(szAnsiLabel));
