...
HRESULT BlendVideo(LPTSTR szFile1, LPTSTR szFile2)
{
    ...
    (void)StringCchCopyW(wFile1, NUMELMS(wFile1), T2W(szFile1));
    (void)StringCchCopyW(wFile2, NUMELMS(wFile2), T2W(szFile2));
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if( SUCCEEDED(hr))
            {
                ...
                // Set two streams to enable mixing both videos
