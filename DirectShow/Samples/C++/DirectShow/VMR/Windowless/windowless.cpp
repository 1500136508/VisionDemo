...
void UpdateMainTitle(void)
{
    ...
    else
    {
        ...
        GetFilename(g_szFileName, szFile, NUMELMS(g_szFileName));
        ...
        USES_CONVERSION;
        (void)StringCchCopy(szRate, NUMELMS(szRate), A2T(szPlaybackRate));
...
void GetFilename(TCHAR *pszFull, TCHAR *pszFile, size_t len)
{
    int nLength;
    ...
    for (int i=nLength-1; i>=0; i--)
    {
        ...
        {
            szPath[i] = '\0';
            hr = StringCchCopy(pszFile, len, &szPath[i+1]);
            bSetFilename = TRUE;
            break;
        }
    }
...
int PASCAL WinMain(HINSTANCE hInstC, HINSTANCE hInstP, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    wc.hIcon         = LoadIcon(hInstC, MAKEINTRESOURCE(IDI_WINDOWLESS));
    if(!RegisterClass(&wc))
    {
