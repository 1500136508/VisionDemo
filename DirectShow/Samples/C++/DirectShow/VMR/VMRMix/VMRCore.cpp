    ...
    m_lpSrcRect(NULL)
{
    ...
    (void)StringCchPrintf( m_szEventStopTest,NUMELMS(m_szEventStopTest),    _T("STOP_TEST_%ld\0"), m_dwID);
    (void)StringCchPrintf( m_szEventCloseWindow, NUMELMS(m_szEventCloseWindow), _T("CLOSE_WINDOW_%ld\0"), m_dwID);
    (void)StringCchPrintf( m_szEventKillCore, NUMELMS(m_szEventKillCore),   _T("KILL_CORE_%ld\0"), m_dwID);
    (void)StringCchPrintf( m_szEventResumeCore, NUMELMS(m_szEventResumeCore),  _T("RESUME_CORE_%ld\0"), m_dwID);
...
HRESULT CVMRCore::CreateGraph()
{
    ...
    for( ncount =0; ncount < m_pML->Size(); ncount++)
    {
        ...
        if (FAILED(hr))
        {
            ...
            hr = StringCchPrintf( szMsg, NUMELMS(szMsg), _T("Error while rendering file, method returned %s\n\0"),
...
HRESULT CVMRCore::AddTestFilter()
{
    ...
    {
        TCHAR szMsg[MAX_PATH];
        (void)StringCchPrintf( szMsg, NUMELMS(szMsg), _T("Error while setting rendering preferences!  hr = %s\n\0"),
                   hresultNameLookup(hr));
        DbgOutString( szMsg);
...
bool CVMRCore::ListFilters()
{
    if (m_pGraph)
    {
        ...
        (void)StringCchPrintf(buffer, NUMELMS(buffer),_T("Filter List (%d found): \n\0"), cFetched);
        DbgOutString(buffer);
        ...
        for (UINT i = 0; i < cFetched; i++)
        {
            ...
            #else
                (void)StringCchCopy(buffer, MAX_PATH-1, W2T(pInfo.achName));
                buffer[MAX_PATH-1] = 0;
            #endif
            ...
            (void)StringCchPrintf(sLogBuffer, NUMELMS(sLogBuffer),_T("(%d) = %s\n\0"), i+1 ,  buffer);
...
LRESULT CVMRCore::OnPaint(HWND hWnd)
{
    ...
    {
        TCHAR szMsg[64];
        (void)StringCchCopy(szMsg, NUMELMS(szMsg), (TCHAR *) hresultNameLookup(hr));
