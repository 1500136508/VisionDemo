...
HRESULT CDemonstration::Perform()
{
    ...
    try
    {
        ...
        if( FAILED(hr) )
        {
            hr = StringCchPrintf( m_szMsg, NUMELMS(m_szMsg), _T("Failed to run VMR, method returned %s\n"),
                       hresultNameLookup(hr));
            DbgOutString(m_szMsg);
            return hr;
        }
    }
    catch(...)
    {
        DbgOutString(_T("Unhandled exception in CDemonstration::Perform()\n"));
        ShellAbort(m_pCore);
        return E_POINTER;
    }
...
HRESULT CDemonstration::Initialize()
{
    ...
    if( m_pDlg->IsBitmapToUse())
    {
        ...
        if(FAILED(hr))
        {
            DbgOutString(_T("Failed to SetAlphaBitmapColorKey() in CDemonstration::Initialize()\n"));
            return E_INVALIDARG;
        }
    }
...
HRESULT CDemonstration::SetAlphaBitmapColorKey(UINT ImageID)
{
    ...
    if( ! m_pCore )
    {
        DbgOutString(_T("Function SetAlphaBitmapColorKey() is called before initializing m_pCore\n"));
        return E_INVALIDARG;
    }
    // prepare the valid default bitmap
    if (FAILED(GetValidVMRBitmap(ImageID)))
    {
        DbgOutString(_T("Unable to get a valid VMRALPHABITMAP\n"));
        return E_INVALIDARG;
    }
    ...
    {
        // Display a message box about the unhandled exception and exit
        ShellAbort(m_pCore);
    }
    return S_OK;
}
...
HRESULT CDemonstration::GetValidVMRBitmap(UINT ImageID)
{
    ...
    if( FAILED(hr) )
    {
        DbgOutString(_T("Failed in GetNativeVideoSize()\n"));
        return hr;
    }
    ...
    if( !hbmpVmr )
    {
        DbgOutString(_T("Failed to load resource\n"));
        DeleteObject(m_hbmp);
        return E_INVALIDARG;
    }
...
HRESULT CDemonstration::UpdateStreams(clock_t tStart)
{
    ...
    for( i=0; i< this->m_MList.Size(); i++)
    {
        ...
        if( !SUCCEEDED(hr))
        {
            hr = StringCchPrintf( m_szMsg, NUMELMS(m_szMsg), _T("Failure in CDemonstration::UpdateStreams, GetMixerControl()->SetAlpha, method returned %s\n"),
                     hresultNameLookup(hr));
            DbgOutString(m_szMsg);
        }
        ...
        if( !SUCCEEDED(hr))
        {
            hr = StringCchPrintf( m_szMsg, NUMELMS(m_szMsg), _T("Failure in CDemonstration::UpdateStreams, GetMixerControl()->SetOutputRect, method returned %s\n"),
                     hresultNameLookup(hr));
            DbgOutString(m_szMsg);
        }
        ...
        if( !SUCCEEDED(hr))
        {
            hr = StringCchPrintf( m_szMsg, NUMELMS(m_szMsg),  _T("Failure in CDemonstration::UpdateStreams, GetMixerControl()->GetOutputRect, method returned %s\n"),
                     hresultNameLookup(hr));
            DbgOutString(m_szMsg);
        }
        else
        {
            ...
            {
                hr = StringCchPrintf( m_szMsg, NUMELMS(m_szMsg),  _T("Failed invalidation of SetOutputRect(): required [l,t,r,b] = [%f,%f,%f,%f] and returned [%f,%f,%f,%f]\n"),
