...
bool CMediaList::Initialize(TCHAR *szFolder)
{
    ...
    do
    {
        (void)StringCchCopy(szCurExt, NUMELMS(szCurExt), szExt);
        ...
        if( psz)
        {
            *psz = 0;
            ...
            if( psz )
            {
                (void)StringCchCopy(szExt, NUMELMS(szExt), psz+1);
            }
        }
        else
        {
            (void)StringCchCopy(szExt, NUMELMS(szExt), _T(""));
        }

        (void)StringCchPrintf(szMask, MAX_PATH-1, _T("%s%s\0"), szFolder, szCurExt);
        szMask[MAX_PATH-1] = 0;
        ...
        (void)StringCchPrintf( szFilePath, MAX_PATH, _T("%s%s\0"), szFolder, fileinfo.name);
        (void)StringCchCopy( pSI->m_szPath, NUMELMS(pSI->m_szPath), (const TCHAR*)szFilePath );

        #ifndef UNICODE
            MultiByteToWideChar(CP_ACP, 0, (const TCHAR*)szFilePath, -1, pSI->m_wszPath, MAX_PATH);
        #else
            USES_CONVERSION;
            (void)StringCchCopy(pSI->m_wszPath, MAX_PATH-1, T2W(szFilePath));
