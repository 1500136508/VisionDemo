...
bool CVMRMixDlg::SetNumberOfStreams( int n)
{
    ...
    (void)StringCchPrintf( szMsg, NUMELMS(szMsg), _T("%d\0"), m_nStreams);
...
void CVMRMixDlg::SelectFolder()
{
    ...
    (void)StringCchCopy(szBuffer, NUMELMS(szBuffer), TEXT(""));
    ...
    while( bFolderIsBad )
    {
        if (GetOpenFileName (&ofn))  // user specified a file
        {
            ...
            TCHAR *psz = NULL;
            (void)StringCchCopy(szTmp, NUMELMS(szTmp), ofn.lpstrFile);
            ...
            {
                (void)StringCchCopy( m_szFolder, NUMELMS(m_szFolder), psz );
                _tcsrev( m_szFolder );
                return;
            }
    ...
    if( bFolderIsBad )
    {
        (void)StringCchCopy( m_szFolder, NUMELMS(m_szFolder), _T("\0"));
    }
    return;
}
...
bool CVMRMixDlg::GetMediaSettings()
{
    ...
    for(n=0; n<pML->Size(); n++)
    {
        ...
        if( false == bRes )
        {
            (void)StringCchPrintf( szMsg, NUMELMS(szMsg), _T("CVMRModule::GetMediaSettings(): failed to clone element %ld, setting source for 5 sec\0"),n);
            DbgOutString( szMsg);
        ...
        if( FAILED(hr) )
        {
            (void)StringCchPrintf( szMsg, NUMELMS(szMsg),_T("*** failed to render source %s, method returned %s\n\0"),
        ...
        (void)StringCchPrintf( szInfo, NUMELMS(szInfo), _T("Source %d: name:%s, duration: %ld\n\0"),
...
HRESULT CVMRMixDlg::RunDemonstration()
{
    ...
    TCHAR szMsg[MAX_PATH];
    (void)StringCchPrintf( szMsg, NUMELMS(szMsg), _T("TIME:: Actual: %ld ms, Expected: %ld ms\n\0"),
