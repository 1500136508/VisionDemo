...
typedef struct SourceInfo
{
...

    {
        ...
        #else
            USES_CONVERSION;
            (void)StringCchCopy(m_wszPath, MAX_PATH-1, T2W(m_szPath));
    ...
    bool CopyTo( SourceInfo* pSI)
    {
        ...
        (void)StringCchCopy( pSI->m_szPath, MAX_PATH, m_szPath );

        #ifndef UNICODE
            MultiByteToWideChar(CP_ACP, 0, (const TCHAR*)m_szPath, -1, pSI->m_wszPath, MAX_PATH);
        #else
            USES_CONVERSION;
            (void)StringCchCopy(pSI->m_wszPath, MAX_PATH-1, T2W(m_szPath) );
            m_wszPath[MAX_PATH-1] = 0;
        #endif
