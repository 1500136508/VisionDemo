...
BOOL VcdPlyerCaptureImage(LPCTSTR szFile)
{
    ...
    if(pMovie)
    {
        ...
        // by the VMR.
        if(pMovie->GetCurrentImage(&lpCurrImage) == S_OK)
        {
            ...
            hr = StringCchPrintf(szText, NUMELMS(szText),TEXT("Captured current image to %s\\%s.\0"), szDir, szFile);
    ...
    )
{
    ...
    StringCchCopy(achFilter, NUMELMS(achFilter),IdStr(STR_FILE_FILTER));
    ...
    )
{
    ...
    */
    ...
    hr = StringCchPrintf(g_achFileName, NUMELMS(g_achFileName),achFileName);
    ...
    if(pMovie)
    {
        ...
        if(SUCCEEDED(hr))
        {
            ...
            hr = StringCchPrintf(achTmp, NUMELMS(achTmp), IdStr(STR_APP_TITLE_LOADED), g_achFileName);
