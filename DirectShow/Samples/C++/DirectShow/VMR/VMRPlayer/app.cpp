...
const TCHAR szClassName[] = TEXT("VMRPlayer_CLASS\0");
        ...
        switch(id)
        {
            case StatusTimer:
            {
                ...
                if(1)
                {
                    ...
                    hr = StringCchPrintf(sz, NUMELMS(sz),TEXT("%s"), FormatRefTime(szFmt, rt, NUMELMS(szFmt)));
    ...
    )
{
    ...
    */
    ...
    else if(flags & MF_SYSMENU)
    {
        ...
        {
            case SC_RESTORE:
                StringCchCopy(szString, NUMELMS(szString), IdStr(STR_SYSMENU_RESTORE));
                break;

            case SC_MOVE:
                StringCchCopy(szString, NUMELMS(szString), IdStr(STR_SYSMENU_MOVE));
                break;

            case SC_MINIMIZE:
                StringCchCopy(szString, NUMELMS(szString),  IdStr(STR_SYSMENU_MINIMIZE));
                break;

            case SC_MAXIMIZE:
                StringCchCopy(szString, NUMELMS(szString),  IdStr(STR_SYSMENU_MAXIMIZE));
                break;

            case SC_TASKLIST:
                StringCchCopy(szString, NUMELMS(szString),  IdStr(STR_SYSMENU_TASK_LIST));
                break;

            case SC_CLOSE:
                StringCchCopy(szString, NUMELMS(szString),  IdStr(STR_SYSMENU_CLOSE));
                break;
        }
    ...
    */
    else
    {
        ...
        {
            StringCchCopy(szString, NUMELMS(szString),  IdStr(item + MENU_STRING_BASE));
        }
    ...
    )
{
    ...
    HRESULT hr = StringCchPrintf(sz, NUMELMS(sz), TEXT("Length: %s\0"), FormatRefTime(szFmt, rt, NUMELMS(szFmt)));
    ...
    )
{
    ...
    HRESULT hr = StringCchPrintf(sz, NUMELMS(sz), TEXT("Elapsed: %s\0"), FormatRefTime(szFmt, rt,NUMELMS(szFmt)));
    ...
    )
{
    ...
    {
        hr = StringCchPrintf(sz,NUMELMS(sz), TEXT("%s\0"),(LPCTSTR) CDisp((LONGLONG) rt,CDISP_DEC));
        return sz;
    }
    ...
    hr = StringCchPrintf(sz, NUMELMS(sz),TEXT("%02d:%02d:%02d h:m:s\0"),hrs, mins, secs);
    return sz;
}
