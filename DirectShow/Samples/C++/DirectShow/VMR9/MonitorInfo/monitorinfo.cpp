...
HRESULT DisplayMonitorInfo(IVMRMonitorConfig9 *pMon)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        hr = StringCchPrintf(szMonitorInfo, NUMELMS(szMonitorInfo), TEXT("This machine has %d attached monitor%s.\r\n\r\n\0"),
        ...
        for (DWORD i=0; i < dwNumMonitors; i++)
        {
            ...
            hr = StringCchPrintf(szInfo, NUMELMS(szInfo)-1,
                     TEXT("Device: %s\r\n")
                     TEXT("Description:  %s\r\n")
            ...
            (void)StringCchCat(szMonitorInfo, NUMELMS(szInfo), szInfo);
        }
