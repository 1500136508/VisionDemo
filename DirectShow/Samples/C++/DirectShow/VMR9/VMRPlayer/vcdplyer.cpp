    ...
    )
{
    ...
    wcsncpy(FileName, T2W(lpFileName), NUMELMS(FileName)-1);
    FileName[MAX_PATH-1] = 0;

    hRes = RenderFileToVMR9( m_Gb, FileName, NULL, FALSE );
    ...
    )
{
    ...
    hres = StringCchCopyW(FileName, NUMELMS(FileName), T2W(lpFileName));
    ...
    if(SUCCEEDED(hres))
    {
        ...
        hres = RenderFileToVMR9( m_Gb, FileName, NULL, TRUE);
        if(FAILED(hres))
        {
            pUnk->Release();
            m_Fg->Release(); m_Fg = NULL;
